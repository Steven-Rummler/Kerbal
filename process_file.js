...
function process_file(reader) {
    science = reader.result.split("name = ResearchAndDevelopment")[1].split("name = VesselRecovery")[0].split("@");
    let experiment = [];
    let location = [];
    let sci = [];
    let cap = [];
    ...
    for (i = 1; i < science.length; i++) {
        ...
        if (i != science.length - 1) experiment.push(science[i].split(" ")[science[i].split(" ").length - 1]);
    ...
    for (i = 0; i < experiment.length; i++) {
        save_data.push({ "experiment": experiment[i], "location": location[i], "sci": sci[i], "cap": cap[i] });
    }
    let updates = 0;
    let d, s;
    for (d = 0; d < dataset.length; d++) {
        for (s = 0; s < save_data.length; s++) {
            if (dataset[d].experiment === save_data[s].experiment && dataset[d].location === save_data[s].location) {
                updates++;
