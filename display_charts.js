function display_charts() {
    ...
    let experiments_chart_data = {
        ...
        series: [{ name: 'Science Completed', data: experiment_data.map(a => a.sci) }, { name: 'Maximum Science', data: experiment_data.map(a => a.cap) }]
    }
    let celestials_chart_data = {
        categories: celestial_data.map(a => a.celestial),
        series: [{ name: 'Science Completed', data: celestial_data.map(a => a.sci) }, { name: 'Maximum Science', data: celestial_data.map(a => a.cap) }]
    }
    ...
    for (let c = 0; c < celestials.length; c++) {
        ...
        let biome_chart_data = {
            ...
            series: [{ name: 'Science Completed', data: biome_data.map(a => a.sci) }, { name: 'Maximum Science', data: biome_data.map(a => a.cap) }]
        };
